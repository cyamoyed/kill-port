...
class KillPortWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if os.name == 'nt':
                self.finished.emit(False, "执行此操作需要管理员权限", "")
            else:
                self.finished.emit(False, "执行此操作需要 root 权限", "")
...
class KillPortGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.kill_button = QPushButton('终止进程')
