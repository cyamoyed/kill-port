...
一个跨平台的端口进程杀死工具，用于快速查找并杀死占用指定端口的进程。支持 Windows、macOS 和 Linux 平台，提供 GUI 和命令行两种操作方式。
...
- ⚡ **快速操作**：一键杀死占用指定端口的进程
- 📋 **详细反馈**：显示操作结果和进程 PID
- 🔒 **权限提示**：当需要管理员/root 权限时给出明确提示
- 🛠️ **自动化打包**：支持生成各平台的可执行文件
...
A: 可能是权限不足导致无法查看所有进程，请尝试以管理员/root 权限运行。

### Q: 为什么在 macOS 上打包后需要创建 DMG 文件？
A: DMG 是 macOS 上常用的应用分发格式，需要在 macOS 系统上使用专门的工具创建。
...
1. **无法找到端口占用信息**：
   - 检查是否有足够的权限
   - 确认端口号是否正确
   - 尝试使用 `netstat` (Windows) 或 `lsof` (macOS/Linux) 手动查看端口占用情况
